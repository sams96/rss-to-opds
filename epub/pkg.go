...
func fixXMLId(id string) (string, error) {
	...
	fileIdentifier := fmt.Sprintf("id%s", uuid.NewString())
...
func (p *pkg) write(w *zip.Writer) error {
	now := time.Now().UTC().Format("2006-01-02T15:04:05Z")
	p.setModified(now)

	pkgFilePath := filepath.Join(contentFolderName, pkgFilename)
	...
	dst, err := w.Create(pkgFilePath)
	if err != nil {
		return err
	}

	_, err = dst.Write(pkgFileContent)
	return err
}
