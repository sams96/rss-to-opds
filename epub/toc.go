...
func (t *toc) addSubSection(parent string, index int, title string, relativePath string) {
	...
	if parent == "-1" {
		...
		parentRelativePath := filepath.Join("xhtml", parent)
		...
		err := AppendNav(t.navXML.Links, parentRelativePath, l)
		if err != nil {
			log.Println(err)
		}
		/*
...
func (t *toc) writeNavDoc(w io.Writer) error {
...
func AppendNav(t []*tocNavItem, parentFilename string, targetsection *tocNavItem) error {
	...
	for _, nav := range t {
		...
		// Recursively check all children of the current section
		err := AppendNav(nav.Children, parentFilename, targetsection)
		if err == nil {
			return nil
		}
	}
